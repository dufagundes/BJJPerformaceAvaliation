...
export function PerformanceOverview({ ratings }: { ratings: RatingDistribution[] }) {
      ...
      <div className="grid gap-6 p-5 md:grid-cols-[220px_1fr] md:items-center">
        <div className="flex flex-col items-center justify-center gap-4">
          <div
            ...
            style={{ background: "conic-gradient(#2563eb 0deg 302deg, #e2e8f0 302deg 360deg)" }}
          ...
          >
            <div className="grid h-28 w-28 place-items-center rounded-full bg-white shadow-inner">
              <div className="text-center">
                <p className="text-4xl font-semibold text-slate-950">4.2</p>
                <p className="text-xs font-medium text-slate-500">Average Rating</p>
              </div>
            </div>
          </div>
          <p className="text-sm text-slate-600">Based on {total} completed evaluations</p>
        </div>
          ...
          {ratings.map((item) => (
            <div key={item.label} className="grid grid-cols-[112px_1fr_32px] items-center gap-3 text-sm">
              ...
              <div className="h-2.5 overflow-hidden rounded-full bg-slate-100">
                <div className="h-full rounded-full bg-indigo-600" style={{ width: `${(item.count / max) * 100}%` }} />
              </div>
